$ (function(){
  ...
  socket.on('connect',function(){
    ...
    document.getElementById("cmd").innerHTML += '<li>'+ data.description +'</li>';
  ...
  socket.on('onlineStack',function(stack){
    $('#list').empty();
    $('#list').append($('<li>').append($('<button id="ubtn" class="btn btn-info btn-block bg-light text-dark"></button>').text("Group").css({"font-size":"18px"})));
    for (var user in stack){
      //setting txt1. shows users button.
      if(user == username){
        var txt1 = $('<button class="btn border-success disabled"> </button>').text(user).css({"font-size":"18px"});
      }
      else{
        var txt1 = $('<button id="ubtn" class="btn border-success btn-md">').text(user).css({"font-size":"18px"});
      }
      //setting txt2. shows online status.
      if(stack[user] == "Online"){
        var txt2 = $('<span class="mt-2 text-success"></span>').text("*"+stack[user]).css({"float":"right","font-size":"18px"});

      }
      else{
        var txt2 = $('<span class="mt-2 text-danger"></span>').text(stack[user]).css({"float":"right","font-size":"18px"});
      }
      //listing all users.
